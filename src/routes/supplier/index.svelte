<script>
	...
	let editIdx;
	let title = '新增';
	let tableHeaders = ['名称', '电话', '状态', '操作'];
	let selected = suppliers.map(() => false);
	...
	async function del(id) {
		await crud(`/supplier/${id}`, { method: 'DELETE' });
	}
	async function submit() {
		if (editOrAdd == 'add') {
			await crud(`/supplier/add`, {
				method: 'POST',
				headers: {
			...
			await crud(`/supplier/${suppliers[editIdx].id}`, {
				method: 'PUT',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	function selectAllChanged(e) {
		selected = suppliers.map(() => e.detail);
	}
</script>
