<script>
	...
	let selected = customers.map(() => false);
	...
	async function submit() {
		if (editOrAdd == 'add') {
			...
			await crud(`/crud/update_${dbTable}_${customers[editIdx].id}`, {
				method: 'PUT',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	function selectAllChanged(e) {
		selected = customers.map(() => e.detail);
	}
</script>
